...
// ExportToOutputs 自动导出小说到 outputs 目录（供 WebDAV 阅读）
func (a *App) ExportToOutputs(novelID int64, format string) error {
	var n novel.Novel
	if err := a.novel.DB.WithContext(a.ctx).First(&n, novelID).Error; err != nil {
		return err
	}
	...
	// 保存到 outputs 目录
	outputDir := filepath.Join(config.DataDirPath(), "outputs")
...
// ExportAllToOutputs 批量导出所有小说到 outputs 目录（供 WebDAV 阅读）
func (a *App) ExportAllToOutputs() error {
	var novels []novel.Novel
	if err := a.novel.DB.WithContext(a.ctx).Find(&novels).Error; err != nil {
		return err
	}
	for _, n := range novels {
		a.ExportToOutputs(n.ID, "txt")
	}
	return nil
}
