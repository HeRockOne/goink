...
单章：prepare → outline → write → cmd-review → maintain → prepare
批量：prepare → outline ⇄ write（循环N章）→ cmd-review → maintain → done
...
1. 加载技能（tech-emotion-injection, tech-chapter-hook-enhanced, tech-maliang-method, tech-dialogue-subtext, tech-chapter-title-hooks；新书首次 outline 加 tech-golden-three-chapters、tech-golden-finger-design，以及对应类型的专精 skill：type-xuanhuan-cultivation/type-urban-martial-arts/type-post-apocalyptic-survival/type-suspense-rule-horror/type-historical-time-travel）
2. **edit**(outlines/NNN.md)（required）— 写大纲（标题/基调/场景/关键事件/重点角色/伏笔操作/章末钩子）
3. 等待用户审批 → **set_phase("write")**

### write

1. 加载技能（tech-show-dont-tell, tech-info-density, tech-pov-purity, tech-anti-ai-writing）
2. **edit**(chapters/NNN.md)（required）— 写正文
3. 校验字数（2500-4000）
4. 记录关键物品出现 → create_item_occurrence
5. **set_phase("cmd-review")**

### cmd-review

1. **run_subagent**(agent_type="cmd-review")（required）— 启动审稿
2. 根据意见修复
...
| 5 | **更新章节计划** | 每章必做 | update_chapter_plan（cmd-next/near/far） |
...
> 每阶段先加载对应技能再执行。manual 模式（cmd-collect/cmd-memory/cmd-next/cmd-review）由用户 `/` 触发，不在此表。

| 阶段 | 技能 |
|------|------|
| **init（开书）** | core-init-phase（开书流程）, tech-genre-templates（12类型）, tech-book-outline（总纲）, tech-character-design（角色设计）, tech-world-building-system（世界观） |
| **prepare（准备）** | tech-common-sense-logic（一致性）, tech-genre-templates, tech-book-outline（卷纲）, tech-brainstorm-composer（卡情节时构思） |
| **outline（大纲）** | tech-book-outline（章节蓝图）, tech-chapter-opening（每章开头）, tech-chapter-hook-enhanced（章末钩子）, tech-maliang-method（打脸/金手指节奏）, tech-dialogue-subtext（对白设计）, tech-emotional-arc（情感弧线）, tech-opening-chapter（第一章开篇） |
| **write（正文）** | tech-show-dont-tell（展示）, tech-info-density（信息密度）, tech-pov-purity（视角）, tech-anti-ai-writing（八条铁律）, tech-shuangdian-pacing（爽点节奏）, tech-climax-scene（战斗章）, tech-foreshadow-cycle（埋伏笔）, tech-pacing-control（节奏控制）, tech-scene-beats（场景节拍）, tech-emotion-injection（情绪注入）, tech-word-count-calibration（字数校准） |
| **write后（自审）** | tech-revision-pass（修改润色）, tech-anti-ai-grade（用词级反AI） |
| **cmd-review（审稿）** | run_subagent(agent_type="cmd-review") → tech-cmd-review-standards（16项判定） |
| **maintain（维护）** | tech-anti-repetition（去重）, tech-foreshadow-cycle（回收伏笔） |
| **完结** | tech-book-completion（完本清单） |

## 硬约束

- 每章至少1个情绪锚点；情绪浓度高时禁止讲述句
- 每章至少1次快慢节奏切换；关键场景必须现场描写≥300字
- 每章至少1个爽点（对照 tech-shuangdian-pacing）；章末必有钩子且类型不与前2章重复
- 禁止功能报告体（连续3段无感官描写）；禁止散文体
- 每章35-55段，每段60-80字，总字数2500-4000
- 字数不足禁止转阶段
- 大战之间必须插非战斗章（对照 tech-climax-scene）
- 完结前检查伏笔回收率≥90%（对照 tech-foreshadow-cycle / tech-book-completion）
