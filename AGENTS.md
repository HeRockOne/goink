...
internal/               # 核心库
├── agent/              # Agent 循环 + 门禁 + 压缩（DESIGN.md 有过时声明，以代码为准）
├── agentcfg/           # 系统提示词 / 工具白名单
├── mcp_tools/          # MCP 工具（description/schema 是创作方法论，禁止精简）
├── cacheprobe/         # 成本模拟（包级状态靠 simMu 串行化）
├── session/            # 消息存储（append-only）
├── skill/              # 技能库，builtin/ 为内置 skill 全量版
├── llm/                # LLM 客户端
└── web/                # web_fetch 网页抓取库（反爬检测 + SSRF 防护）
frontend/               # React 前端
└── src/lib/wailsjs/    # Wails bindings，自动生成，禁止手改
cmd/cacheprobe/         # 成本模拟 CLI
skills/                 # 用户级 skill 真源（版本控制，同名覆盖内置）
migrate/                # 数据库自动建表（新增表加在 models 数组）
docs/                   # 文档：architecture / design / adr / archive
build/                  # 打包配置
```

跨层契约：

- 前端只通过 Wails bindings（`frontend/src/lib/wailsjs/`）调 Go 能力，不手改生成文件；新增/修改 app 方法或结构体后的流程见"Wails 开发规范"。
- skill 三层结构：内置（`internal/skill/builtin/`）、用户级（`skills/`，同步到 `~/.goink/skills/` 后生效）、小说级；同名优先级：小说级 > 用户级 > 内置。
- 数据目录在 exe 同级 `D:\Goink\`，含 `novel-agent.db`、`novels/`；每本小说独立 Git 仓库在 `{DataDir}/novels/{id}/`，含 `chapters/NNN.md`、`outlines/NNN.md`。
- 消息 append-only，`to_api`/`to_frontend` 独立控制可见性；压缩时递增 `active_version`，不删旧消息（细则见"数据库约定"）。
...
1. **创作质量第一**：任何 token/性能优化，先问"创作质量会受影响吗"，会则不做；细则见"创作质量红线"。
2. **全量工具定义 + 稳定前缀是 ADR-0001 的刻意设计**（缓存命中率 89-93%）：改工具注入/消息顺序前必须读 `docs/adr/0001-prompt-caching.md`。
3. **并发代码警惕共享状态**：Agent 单例字段/包级变量在多会话并发时会串扰（教训：agent 门禁 phaseGate 存 Agent 字段导致桌面+移动端并发互相覆盖，已改为 Run 局部变量；cacheprobe 包级状态靠 simMu 串行化）。新写并发路径时优先局部变量/context 传递。
...
1. 所有表通过 `migrate/migrate.go` 自动建表，新增表加在 `models` 数组中
2. `model_usage` 表存模型级 token 累计。累计时传**增量值**，不是累计值
3. 消息是 append-only，`to_api`/`to_frontend` 独立控制可见性
4. 角色关系追加式，`is_current` 标记当前状态
5. 时间线 `target_chapter` 仅用于 ORDER BY，不用于 WHERE
6. 压缩时递增 `active_version`，不删旧消息

## Wails 开发规范与经验

> 改动 CGO 编译、Wails bindings、计费统计相关代码前必读。

1. **Windows 编译必须带 CGO 环境**：ONNX/sqlite-vec 用 `//go:build cgo`。`build.ps1` 已设置（PATH 含 MSYS2 mingw64、`CGO_ENABLED=1`、`CGO_CFLAGS=-I$(go env GOMODCACHE)/github.com/mattn/go-sqlite3@版本`——sqlite-vec 的 C 代码 include "sqlite3.h"，头文件由 mattn 包自带）。直接用 `go build ./...` 不带这些环境变量报 `sqlite3.h: No such file or directory` 是缺 include 路径，不是"Windows 不能编译"
2. **bindings 自动生成**：`frontend/src/lib/wailsjs/`（App.js/App.d.ts/models.ts）由 `wails generate module` 生成，**不手改**。新增/修改 app 方法或结构体后，先跑 `wails generate module`（需 CGO 环境）再构建前端，否则前端调不到新方法
3. **计费有关的缓存字段**：优先 `prompt_tokens_details.cached_tokens`，fallback `prompt_cache_hit_tokens`。详见 `docs/archive/billing-panel.md`
4. **`updateUsage` 在 `tokens.go`，每次 EventUsage 触发**。改这里要小心 `perModel` 和全局累计值的一致性
...
|------|------|
| 编译 | `go build ./...` |
| Go 单测（e2e 需真实 git/ONNX 环境，可跳过） | `go test ./internal/... ./app/...` |
| internal/web 纯逻辑测试 | `go test ./internal/web/... -run TestIsEncodingGarbled\|TestIsAntiCrawl\|TestCompressionRatios` |
| 前端构建（前端改动必须跑） | `cd frontend && npm run build` |

- 以上命令在项目根执行，需先设置 CGO 环境（见"Wails 开发规范"）。
- **例外：`internal/web` 测试必然失败，与本轮改动无关，勿纠结**，原因：
  ...
  - 处理：整体测试时跳过该包；纯逻辑测试（`fetch_internal_test.go` 的乱码/反爬判定）正常，可单独跑上表第三行命令
