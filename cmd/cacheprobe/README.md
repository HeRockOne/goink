...
> **模拟器定位（2026-08-16 明确）：严格门禁阶段规范基准**——plays 建模"LLM 严格按
> 门禁配置与 kernel 规范执行"的成本（完整 prepare/outline/write/review/maintain、每章
> miniMaintain 六件套、批量每 3 章自检、批末全批审稿）。真机会话是 LLM 实际行为：
> 低于模拟 = 偷工减料（软约束失效、缺大纲、miniMaintain 2/6），高于模拟 = 发癫
> （思考重复循环、超长思考、拖沓重试、人工介入）。**个别 LLM 的发癫行为不纳入模拟**，
> 对标时先剔除（如批量会话 ~35K 抽风输出）；模拟器的价值 = 预测"按规则做该花多少"。
...
> 剔抽风 ¥0.083，真机低 18% = LLM 偷工减料：批量会话跑在硬约束化前，miniMaintain
> 2/6、自检未触发，未做完整维护）；续写单章 out 13.2K vs 真机 12.8K（差 3%）、
> miss 91.1K vs 53.5K、成本 ¥0.261 vs ¥0.104（真机低 2.5 倍 = 第四章是旧版软约束
> 精简流程：20 工具调用/prepare 3 查询/无大纲 edit/miniMaintain 2/6——**模拟器预测
> 严格门禁下的应有成本，真机偷工减料不在对标误差内**；新版硬约束真机单章待测，
> 预测会向模拟器收敛）。
...
> （对标真机第四章 53.5K/12.8K，输出差 3%；miss 真机低 1.7 倍 = 旧版精简流程未做
> 全套查询）；续写批量（历史 4 章）miss 124.1K vs 真机 167.0K（真机高 26% = 抽风
> 重试/拖沓请求）、out 27.8K vs 真机剔抽风 ~25.7K（差 8%）。
...
结论：
- 模拟器 = **严格门禁规范基准**：预测"LLM 严格按门禁与 kernel 规范执行"的应有成本。真机低于模拟 = 偷工减料（软约束失效、缺大纲、miniMaintain 2/6），高于模拟 = 发癫（抽风思考、拖沓重试、人工介入）——个别 LLM 发癫不纳入模拟，对标时先剔除。
- 批量模式每章成本最低（新会话 ¥0.055/章 low 档；续写批量历史 4 章 ¥0.098/章，对标新版硬约束真机）；单章轮内历史累积使后续章略贵（¥0.18-0.26/章）；续写场景只统计目标增量。
- 模拟口径已多次校准（技能重复计数 / 查询 size / 首轮被动缓存 / 大纲与子代理 / 续写场景与分组并行），输出口径与真机差 3-8%（思考/正文/大纲准确）；输入侧差距全部可归因于真机 LLM 行为（偷工减料或发癫），非模拟失真。
