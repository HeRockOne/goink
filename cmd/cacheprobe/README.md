...
| 续写单章（历史 3 章） | 0 | 0 | 0 | 19094545 | 110449 | 13131 | 99.4% | 0.5186 | 0.5186 |
| 续写批量 5 章（历史 4 章） | 0 | 0 | 5 | 59801002 | 170236 | 39572 | 99.7% | 1.4454 | 0.2891 |
...
> output/miss 分类从基线截断）。实测 low 档：续写单章（历史 3 章）miss 110.4K/out 13.1K，
> 对标真机第四章 53.5K/12.8K——输出差 2%（思考/正文口径准确），miss 高 2 倍 = 模拟器
> 规范全流程每阶段重复查询（prepare/review/maintain 全套 ~24 查询）vs 真机并行合并 +
> 省 token 参数；续写批量（历史 4 章）miss 170.2K vs 真机 167.0K（差 1.9%）、out 39.6K
> vs 真机剔抽风 ~25.7K（高 54% = 模拟每章 6 段 edit + 完整审稿的规范假设）。
