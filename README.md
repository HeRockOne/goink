...
- 5 阶段校验：prepare → outline → write → cmd-cmd-main-review → maintain
- 每阶段有 tools 白名单 + require 必调列表
- maintain 阶段 15 项逐项检查清单（详见 `core-core-main-writing-kernel.md`）
- 门禁配置存数据库，不占 AI 上下文
...
GET  /api/search-cmd-cmd-main-memory        语义搜索
...
| core-core-main-writing-kernel.md | 15 项 maintain 检查清单 |
