...
桌面端设置页二维码为移动端页面完整 URL，格式：
```
http(s)://<ip>:<port>/mobile/?token=<token>
```
- 系统相机/浏览器扫码识别为 URL 直接打开移动端页面，页面自动写入令牌并清理地址栏
- 移动端设置页内"📷 扫描二维码"同样支持，扫码后自动写入服务器地址与令牌并验证 `/api/health` 连接
- 兼容旧格式（`goink://` 协议串、纯 32 位令牌字符串）
