...
- 行为开关（2026-08-09 系统性重构）：inject/inject_dedup/same_phase/word_count_check/
  word_count_reset/mutating_guard 显式声明，缺省=legacy 行为（存量配置零迁移）；
  自定义阶段名不再依赖 "write" 硬编码；同阶段 set_phase 的"章边界打卡"语义由 same_phase 显式控制
- 门禁不自动推进阶段（自动推进已删除）：阶段切换必须 LLM 主动 set_phase——设计契约，
  曾有过"require 满足即自动推进"实现（失败时假成功 reminder + 技能记错阶段 + 与批量显式循环冲突），已移除
- 质量节奏（定案，性价比 87.7 全场第一）：每 3 章自检（**一致性重点**：get_characters/get_timeline/
  get_writing_snapshot 读状态，对照最近 3 章正文查设定矛盾——角色状态/时间线/伏笔/章节衔接/重复，
  普通读者抓不住文笔但设定矛盾一眼穿帮；**文笔次重点**：read revision-pass + anti-ai-grade 查节奏/AI 味；
  发现问题立即 edit 修复；不调 run_subagent 不 set_phase——write 阶段白名单无 run_subagent）
  + 批末 review 阶段 run_subagent 审读本批全部 N 章（子代理 fork 完整主历史可见全部正文），
  一致性优先，逐章 read 自查 + edit 修复 + 字数复查，不要只审第 1 章
- write 显式循环 + 注入去重（2026-08-09 定案）：每章结束 set_phase("write") 声明下一章边界
  （同阶段切换幂等成功，只产生显式阶段记录，零校验开销）；系统自动注入已去重——injectPhaseSkills
  只注入本阶段缺失的必读技能（missingInjections），已注入过的不重复注入，每章 set_phase 无重复注入成本；
  上下文压缩后门禁清空技能记录，需按需补读（auto_skill_injection 或下次 set_phase 自动注入）
- 不要拆成多批：批次边界有新窗口成本（2批×3章 ¥0.112/章 比 单批6章 ¥0.090/章 贵 24%）
- 批量对比：定案方案 ¥0.1027/章（显式循环版）vs 单章 5 轮 ¥0.278/章（省 63%）；门禁配置零改动 -->
<!-- phase-gate-config
mode: single
phase: init
