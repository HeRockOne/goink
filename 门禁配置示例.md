...
- 质量节奏（定案，性价比 89.7 全场第一）：每 3 章自检（**一致性重点**：get_characters/get_timeline/
...
- write 循环内禁止重复 set_phase("write")：同阶段 set_phase 直接成功且会重复注入技能全文
  （SetPhase 同阶段放行 + injectPhaseSkills 无条件注入），浪费 token 且挤占注意力
