...
import { Activity, Play, Loader2, TrendingUp, Plus, RotateCcw, X, Gauge, Layers } from 'lucide-react'
...
function modeOf(sc: SimScenario): 'single' | 'batch' | 'mixed' {
  if (sc.batch_chapters > 0 && sc.gate_rounds === 0) return 'batch'
  if (sc.gate_rounds > 0 && sc.batch_chapters === 0) return 'single'
  return 'mixed'
}
...
function chaptersOf(sc: SimScenario): number {
  const m = modeOf(sc)
  if (m === 'single') return Math.max(1, sc.gate_rounds)
  if (m === 'batch') return Math.max(1, sc.batch_chapters)
  return Math.max(1, sc.gate_rounds + sc.batch_chapters * sc.batch_rounds)
}

const MODE_LABEL: Record<string, string> = { single: '单章', batch: '批量', mixed: '混合' }
    ...
    <main className="flex-1 min-w-0 overflow-y-auto overscroll-contain bg-background">
      <div className="max-w-6xl mx-auto px-8 py-6 space-y-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text-foreground">写书成本模拟</h1>
          <span className="text-xs text-muted-foreground">无网络、无 LLM 调用，按真实对话结构模拟缓存命中与费用</span>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-lg border bg-card px-4 py-3">
          ...
          <span className="text-xs text-muted-foreground tabular-nums">
            缓存 ¥{prices.cache.toFixed(2)}/M · 输入 ¥{prices.input.toFixed(2)}/M · 输出 ¥{prices.output.toFixed(2)}/M（来自设置）
          </span>
            ...
            {scenarios.map((sc, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  单章
                  {numInput(sc.gate_rounds, n => updateScenario(i, { gate_rounds: n }))}
                </span>
                <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  短对话
                  {numInput(sc.short_qa_rounds, n => updateScenario(i, { short_qa_rounds: n }))}
                </span>
                <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  批量章
                  {numInput(sc.batch_chapters, n => updateScenario(i, { batch_chapters: n }))}
                </span>
                <span className="flex items-center gap-1.5 text-[10px] text-muted-foreground">
                  批量轮
                  {numInput(sc.batch_rounds, n => updateScenario(i, { batch_rounds: n }))}
                </span>
                <span className="w-12 text-[10px] text-muted-foreground/70">{MODE_LABEL[modeOf(sc)]}</span>
                <button
        ...
        {tab === 'compare' && (
          <div className="rounded-lg border bg-card overflow-hidden">
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-muted-foreground border-b border-border">
                    ...
                    <th className="text-right py-2 font-normal">模式</th>
                ...
                <tbody>
                  {results.map((r, i) => {
                    const perCh = r.cost / chaptersOf(scenarios[i] ?? { gate_rounds: 1, short_qa_rounds: 0, batch_chapters: 0, batch_rounds: 1 })
                    ...
                    return (
                      <tr key={i} className={`border-b border-border/50 ${isCheapest ? 'bg-amber-50/60' : ''}`}>
                        ...
                        <td className="py-2 text-right text-muted-foreground">{MODE_LABEL[r.mode]}</td>
            ...
            <p className="text-[10px] text-muted-foreground/70 px-4 py-2 border-t border-border">
              每章成本 = 总成本 ÷ 产出章数（单章=轮数，批量=章数，混合=单章轮 + 批量章×批量轮）。最省行高亮，适合找最优工作负载组合。
            </p>
          </div>
        )}
        ...
        {tab === 'miss' && (
          <div className="rounded-lg border bg-card overflow-hidden">
            {results.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-10">点击"跑全部场景"生成 miss 构成表</p>
            ) : (
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-muted-foreground border-b border-border">
            ...
            <p className="text-[10px] text-muted-foreground/70 px-4 py-2 border-t border-border">
              ...
              查询=get_*/search_*，固定/NS=系统提示与小说设定，正文/大纲=写作正文与大纲消息。定位成本大头。
            </p>
          </div>
        )}
        ...
        {tab === 'scale' && (
          <div className="rounded-lg border bg-card p-4 space-y-3">
            {results.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-10">点击"跑全部场景"，再选场景看窗口刻度</p>
            ) : (
              <>
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-muted-foreground">{MODE_LABEL[results[Math.min(scaleIdx, results.length - 1)].mode]} 模式</span>
                </div>
