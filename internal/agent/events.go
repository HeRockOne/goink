...
const (
	...
	EventRetry                              // 自动重试通知（临时，前端几秒后消失）
	EventPhaseGate                          // 阶段门禁状态变化
)
...
type AgentEvent struct {
	...
	RetryCount       int            `json:"retry_count,omitempty"`       // EventRetry: 当前第几次
	RetryMax         int            `json:"retry_max,omitempty"`         // EventRetry: 最大次数
	RetryWait        int            `json:"retry_wait,omitempty"`        // EventRetry: 等待秒数
	PhaseGate        *PhaseStatus   `json:"phase_gate,omitempty"`        // EventPhaseGate: 阶段门禁状态
...
// String 返回 AgentEventType 的字符串表示，用于 WebSocket 广播。
