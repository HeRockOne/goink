...
var phaseThinkingBoundary = map[string]string{
	...
	"write":    "thinking 硬预算 ≤500 字。只允许：①按大纲拆段并给每段分配字数（总和=NS 字数范围）②对照已注入必读技能列 3-5 条自检雷区。绝对禁止在 thinking 写任何叙事正文/描写/对话/摘要——哪怕一句。正文 100% 直接写进 full_replace 的 content 参数；thinking 出现 ≥2 行连续叙事文字即判违规（浪费的 token 不计入有效创作）。成稿后 get_chapter_list 核字数→check_story_consistency；建档/时间线维护留 maintain",
