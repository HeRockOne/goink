...
当 always-mode 技能中包含 phase-gate-config 配置时，阶段门禁自动激活。门禁规定了每个阶段允许使用的工具和必须完成的工具。

- 门禁始终激活，自动进入第一个阶段（如 prepare）
- 每个阶段有 tools 列表，只能使用允许的工具
- 每个阶段有 require 列表，必须调用过列表中的所有工具后才能切换到下一阶段
- 切换阶段：调用 set_phase({"phase":"目标阶段名"})
- require 未满足时 set_phase 会被阻塞，提示缺少哪些工具调用
- **不自动推进**——你必须主动调 set_phase 切换阶段
- 如果没有门禁配置，set_phase 无操作

⚠️ **强制要求**：每个阶段完成后，你必须立即调用 set_phase 切换到下一阶段。这是你最重要的操作——不调用 set_phase 会导致整个流程卡死。阶段流程如下：

| 阶段 | 完成条件 | 必须调用 |
|------|---------|---------|
| prepare | 五门检查通过 | set_phase("outline") |
...
每个阶段结束时，检查 require 是否满足，满足后**立刻**调用 set_phase。不要等，不要忘，不要跳过。

创作流程：prepare（五门检查）→ set_phase("outline") → outline（写大纲）→ set_phase("write") → write（写正文）→ set_phase("review") → review（审读）→ set_phase("maintain") → 维护 → set_phase("prepare") 回到准备。

非创作场景（修改弧线、调整大纲、讨论剧情等）不需要激活门禁，直接使用工具即可。
