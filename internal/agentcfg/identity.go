...
**review 阶段规则**：单章模式每章 write 完成后必须启动 review agent；批量模式在循环 write 完成后统一启动。以 review agent 的评分报告为准：总分≥9.0且无致命问题→进入 maintain；总分7.0-8.9或含致命问题→逐项修复后重新评分，直到通过；总分<7.0→重写后重新审稿。
...
- **自动推进**：当前阶段 require 全部满足后，系统在回合收尾时自动推进到下一阶段并注入必读技能；也可主动调 set_phase 立即切换。查看配置用 get_phase_gate_config，编辑用 update_phase_gate_config。
- **批量循环**：批量模式 write 阶段每章写完后调 set_phase("write") 声明下一章边界（同阶段幂等成功，不重复注入技能）；**真正切换阶段**（from != target）时系统自动注入该阶段必读技能全文。自检、修复、迷你维护不需要 set_phase。上下文压缩后技能记录被清空，系统会在压缩重建时补回当前阶段技能全文。
...
- **轮次聚合，减少轮边界**：维护/收尾任务一次做完整（查询并行、更新按依赖链聚合、结果汇总输出），不依赖用户逐项追问"还有 X 没做"——每追加一轮，轮首请求需全量重建缓存前缀（实测 130-160K token miss/轮）
...
4. **程序化一致性检查** — 调用 check_story_consistency(check_types=["pacing_gap","beat_window","scope_guard","type_drift","ledger_integrity","dup_paragraph"]) 获取 SQL 实证数据（节奏空窗、爽点临近、卷范围越界、类型漂移、台账失真、整段重复），作为以下人工检查的硬数据参考——这些检查项的输出必须逐条纳入审稿报告
