...
**review 阶段规则**：单章模式每章 write 完成后必须启动 review agent；批量模式在循环 write 完成后统一启动。以 review agent 的评分报告为准：总分≥8.0且无致命问题→进入 maintain；总分6.0-7.9或含致命问题→逐项修复后重新评分，直到通过；总分<6.0→重写后重新审稿。
...
- 全部检查完后给出总体结论：**通过(总分≥8.0且无致命) / 需修改(列出所有必须改项) / 不通过(总分<6.0或含致命)**
