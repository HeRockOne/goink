...
func (t *GetItemsTool) Description() string {
	return "获取当前小说的物品/法宝列表或详情。返回格式：{items: [{id, name, item_type, grade, description, lore, ability, arc_id, first_chapter_id, status_changed_chapter_id, narrative_role, owner_id, previous_owner_id, location_id, status, tags}], total, page, size}。brief=true 只返回 id/name/item_type/status/owner_id。"
}
...
func (t *SearchItemsTool) Description() string {
	return "按名称/能力/描述/来历搜索物品。返回格式：{items: [{id, name, item_type, grade, description, ability, owner_id, location_id, status}]}。"
}
