...
type EditArgs struct {
	...
	OldContent string `json:"old_content" jsonschema:"description=要查找的原文片段（search_replace 时必填，与 search_text 等价，二选一）。请从文件中精确复制" validate:"omitempty"`
...
func (t *EditTool) Execute(ctx context.Context, args any, tc ToolContext) (*ToolResult, error) {
	...
	if a.ChangeType == "full_replace" && isChapterPath(a.Path) && len([]rune(proposed)) > 500 {
		...
		injects = append(injects, InjectMessage{
			Role:    "user",
			Content: fmt.Sprintf("你刚刚完成了第%d章的全量替换。请先完成本章剩余步骤：check_story_consistency 一致性核对 -> set_phase(\"review\") 启动审稿（维护操作在 maintain 阶段统一做，write 阶段调用维护工具会被门禁拦截）。", chapNum),
		})
	}
...
func applyChange(a *EditArgs, current string) (string, error) {
	...
	case "search_replace":
		// old_content 与 search_text 等价，二选一（模型习惯用 old_content）
		if a.SearchText == "" && a.OldContent != "" {
			a.SearchText = a.OldContent
		}
		if a.SearchText == "" {
			return "", fmt.Errorf("search_replace 模式需要提供 search_text 或 old_content")
		}
...
3. **search_replace** — 查找并替换指定文本。search_text（或 old_content，二选一）为要查找的原文片段（请从文件中精确复制），new_content 为替换后的文本。replace_all=false（默认）仅替换第一个匹配项，replace_all=true 替换所有匹配。如果连续两次 search_replace 因"未找到匹配"失败，直接用 line_range_replace 代替——不要在同一种模式上反复重试。
...
【省token指令】必须遵守：
1. 禁止使用 full_replace——它发送整个文件（~8000 token）
2. 优先用 search_replace——只发送查找文本+替换文本（~50 token）
3. search_replace 失败时用 line_range_replace——只发送新内容
4. 编辑前先 read 目标行范围，确认内容后再 edit
5. 不要在同一种模式上反复重试，失败就换模式`
...
【省token指令】必须遵守：
1. 禁止不带 start_line/end_line 的全量读取——每行约3-5 token，2000行≈8000 token
2. 只读你需要的行范围，例如：read(path="chapters/001.md", start_line=50, end_line=100)
3. 如果只需要某一段内容，用 start_line/end_line 精确定位，不要读全文
4. include_lines=false 可节省行号token（每行省3个token）`
