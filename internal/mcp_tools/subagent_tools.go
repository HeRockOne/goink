...
【省token指令】必须遵守：
1. instruction 要简洁明确，只描述核心任务，不要写背景说明
2. review 类型：在 instruction 中指定要审读的章节号，子 Agent 会自动读取该章+前1章+角色+时间线
3. memory 类型：在 instruction 中指定查询目标，子 Agent 会自动检索相关数据
4. 子 Agent 的上下文 = 完整主会话历史原文 + 尾部追加子 Agent 身份/指令（fork 模式）——主历史中的正文、设定、NS 直接可读，子 Agent 不需要（也不应）重复 read 主历史里已有的内容；它的工具循环在尾部增长，与主历史互不污染

【报告要求】报告聚焦结论与影响范围（哪些章需要改、改什么、为什么），不要复述正文；长报告优先列问题清单而非逐句分析。`
