...
目前 Agent 通过读取 `goink.md`（纯文本文件，AI 自维护）来获取上下文。问题：
- 纯文本 → Agent 每次都要全文重读，token 成本高
- AI 自维护 → 格式不一，有时过期，有时被覆盖
- 不可查询 → 无法回答"最近 5 章有哪些角色出场"

Writing Snapshot 用结构化方式解决这个问题，**不替代 goink.md**（保留 AI 自由书写的空间），而是作为 goink.md 的补充——快速摘要，精确查询。
...
| `DetailedState` | 自由 Markdown，AI 想写什么写什么，goink.md 的替代/补充 | Agent 自维护 |

## 与 goink.md 的关系

goink.md 不废除。Writing Snapshot 是结构化补充：

```
goink.md（AI 自由书写）:
  "第35章写完了。主角在试炼中获得了天玄剑，但感到剑中有异样气息。
   下一章计划写长老发现剑的异常，引出宗门内部的矛盾。
   注意：副帮主最近行为可疑。"
...
Agent 写下一章前快速读取 Snapshot（几个字段，< 200 token），需要细节时再读 goink.md。
