...
type ItemOccurrence struct {
	...
	ItemID      int64     `gorm:"column:item_id;not null;index;constraint:OnDelete:CASCADE" json:"item_id"`       // FK → items.id
	ChapterID   int64     `gorm:"column:chapter_id;not null;index;constraint:OnDelete:CASCADE" json:"chapter_id"` // FK → chapters.id
