...
> 2026-08-26：Ch36 创作测试全链路审计（sess_2_18cf6e541521b668 / mimo-v2.5 / done / 2807字 / 命中率95.82%）。消息分布 assistant27/system10/tool56/user16 正常。阶段流 prepare→outline(10308成功)→write→review→maintain→done；门禁 require 拦截 3 次（10301 缺 get_chapter_list / 10356 缺 check_story_consistency / 10386 缺 get_item_occurrences+get_character_relations）+ whitelist 拦截 1 次（10355 主代理直调 submit_review）全部正确，模型自纠。审稿 #18=9.0/pass 五维全 9.0、0 致命，报告引用 check_story_consistency 仅 beat_window 提醒（Ch38 大爽点临近），11 类检测+方向锚（NS 10287 进度35 / 10327 进度36 均含【方向锚】第一卷1-40章）均生效。台账防腐已清零（resolved time_entries 越界数 13→0）。缓存命中 95.82% 健康（冷启动 34517 + 注入驱动 16524@10300 / 8177@10319 三峰，符合预期）。异常项已排查（非缺陷）：system 消息 10325 内容+metadata 全空，实为 app/chat.go:332 用户中断标记（event_type=user_stopped；FriendlyError(context.Canceled) 按 internal/agent/errors.go:13 设计返回空串，面板改显"已停止"而非错误）。同刻空 assistant 10324 为中断草稿轮残留（用户 10326 发"别打草稿了，直接写！！"触发，模型 write 阶段 thinking 过度构思）。均无代码缺陷，不需修复。

> 2026-08-26：DeleteSession 回合中止（Ch35 审计驱动）。Ch35 全链路审计发现"幽灵会话"：mimo-v2.5 会话 20:06 开始 Ch35 创作，20:13 被用户删除（sessions 行消失）但 agent 循环继续跑完（编辑章节+审稿落库+git commit）；20:13 用户开 ox-alpha-free 新会话写同一章——两个会话并行写同一章，审稿记录 #16(8.2) 和 #17(8.65) 分别属两个会话，"修复循环"叙事不成立（实为不同会话评分）。根因：`DeleteSession` 只删 DB 不中止运行中的 turn，`agent.Cancel()` 移除 cancels 条目后 `IsRunning()` 立即返回 false。修复：CancelManager 新增 `done map[string]chan struct{}` — `RegisterShutdown`（在 Chat() 中 RegisterCancel 之后调用）创建通道，`Unregister`（Chat() defer 释放时）close+delete；`WaitForStop(key, timeout)` 阻塞等待通道关闭（非 IsRegistered）。`DeleteSession` 改为：Cancel → WaitForStop(10s) → delete。handleSelectSession/isRunning 相关路径不受影响。cancel.go + agent.go + chat.go 三文件。

> 2026-08-26：TPS 实时速率 + 会话切换运行态恢复 + 工具结果持久化（前端体验三项）。① TPS：新 tokenRate.ts（纯前端估算，零后端/事件契约改动）——ChatPanel applyAgentEvent 把 Thinking/Content delta 喂给 tracker，CJK≈0.6 token/字 + 其他≈0.25/字符，3s 滑动窗口瞬时速率 500ms 节流经 onTps 上抛 WorkspaceView→StatusBar→ContextRing bar 模式，显示在命中率左侧圆点相隔；>2s 无 delta（工具执行）速率隐藏且窗口重开（停顿不计入分母，业界惯例佐证 backend.ai-webui#7822/tok-rate-footer）；回合结束 finish() 定格平均速率（仅累计活跃输出时间）。② 会话切换运行态不同步修复：handleSelectSession 无条件 setIsLoading(false)，切回仍在流式的会话时重挂监听（pendingResub 机制）但按钮停留在"发送"态——加载 effect resub 分支补 setIsLoading(true) + 重建的 hist_ turn 状态复位 streaming；回合真正结束时原 send 的 finally（activeCount 归零）恢复按钮。③ 工具结果历史不可查修复：buildToolDisplay（display.go）原先只对 web_search/web_fetch 落库 result，其余工具切换会话/重建后详情按钮消失（expandable 依赖 result/error）——现在全部工具结果序列化为 JSON 截断 4000 字符（与前端展示截断一致）落 extra_metadata.tool_displays，web_* 保留原始 shape 供富文本卡片；前端 subagent 路径（ChatPanel 子代理 ToolCall 处理 + types.ts rebuildTurns）补 result 捕获/恢复。
...
> 2026-08-26：初始阶段清单注入（Ch33 审计驱动）。根因：阶段提醒历来只在 set_phase 转场时由 autoAdvancePhase 注入，prepare 作为初始阶段没有转场、永远收不到清单和思考边界——模型裸奔进 prepare 提前做下一阶段的活（实测：读不存在的 outlines/033.md、15313 字符 thinking 里构思章节标题、直接 edit 写大纲被门禁拦截）。修复：agent.go 轮首（门禁关闭提醒块后）检查历史中是否已有"【当前阶段：X】"标记，缺失则注入当前阶段 phaseChecklist（按 marker 去重，仅主 agent+门禁开启时）；prepare/outline 阶段额外附"本章大纲与正文均未产出，prepare 只做数据收集，禁止预做后续阶段产出"。kernel prepare 段加同义说明（builtin→skills/→~/.goink/skills 三向同步）。同日确认 submit_review 主白名单归属：源码仅在 reviewAgentTools（identity.go），主 agent 直接调用属构建瞬态，方案 A 维持仅子代理可调。

> 2026-08-26：阶段思考边界 + 脆弱性审计 P1-P5/N5/N7 修复。①思考边界（用户观察：prepare 想大纲、outline 想正文——下阶段技能未注入时构思无方法论指导，转场后重想=双倍浪费）：phase_gate.go 新增 phaseThinkingBoundary 常量表，phaseChecklist 按 phase 追加"思考边界"行（代码侧不受 DB 配置快照影响）；kernel 执行纪律加第 6 条，三向同步。②P1 节奏债强制偿还（Q1）：appearance_tools.go 导出 TypeDriftStreak(db,novelID,beforeChapter,genre)（复用 getActionTags 标签体系回看 5 章）；autoAdvancePhase 进 write 时连续 ≥3 章无动作 → 注入"本章必须 ≥300 字高密度场景否则审稿打回"硬提醒。③P2 错误透传+真禁用（N3，佐证 arXiv 2606.05037/hermes-agent#536）：Registry.Execute 的 execErr 路径改为透传真实错误（原统一屏蔽为"服务器内部错误"剥夺模型自纠信号）；agent.go failCnt≥3 后真正跳过执行（原只发提醒不禁用）。④P3 流中重试防重复执行（N1，佐证 tianpan.co 幂等性问题/stripe#402）：新增 executedTools 标记，流中已执行工具后禁止 retryable 重试（重启流会重新生成响应→非幂等副作用翻倍）。⑤P4 工具超时（N2，佐证 MCP/AutoGen 惯例）：Execute 包 context.WithTimeout 3 分钟，run_subagent 豁免（审稿实测 5-8 分钟且内部流有 idle 保护）。⑥P5 consistencyErrors 入 SaveState/LoadState（N4 跨 turn 记忆丢失）+ 往返测试。⑦N5 删除死代码 SaveWordCount/LoadWordCount；N7 chat.go cancel 注册改无条件注销。

> 2026-08-26：审稿评分结构化提交（submit_review）。痛点：ParseReport 正则从 markdown 正文"猜"8 个数，11 条记录 3 次解析事故；模型心算加权从未被校验——测试样例实锤：样例报告自称总分 8.5，实际加权 8.45。方案（最小化两轮收敛后）：新工具 submit_review 只收 8 个原始数（chapter_start/end + 五维分 + fatal_count），代码算总分（权重硬编码 review.go Weight* 常量，与 standards.md 权重表互为锚点）、推导结论（无致命且≥9.0=pass / <7.0或含致命=fail / 其余 revise）；工具不写库（只读分类不变）；RunSubAgent 扫描子代理转录中最后一次 submit_review 调用参数直接建记录，未调用回退 ParseReport；返回文本末尾追加规范行"[审稿结论] 总分：X.X/10（结论）"，门控 reviewVerdictRe 改取最后一个匹配——verdict gate 从此不受正文排版影响。零 DB 迁移、零面板改动。注册：registry.go / identity.go reviewAgentTools / display.go；提示词 identity.go 步骤6-7 + sub-tech-review-standards.md 输出格式节（去掉手写加权算式要求），builtin→skills/→~/.goink/skills 三向同步。

> 2026-08-26：Ch31 审计驱动子代理身份崩塌修复（A+C）。根因：run_subagent fork 父代理完整历史，历史尾部的【当前阶段：review】阶段清单诱导弱模型（hy3/MiniMax-M3）模仿父代理行为——调用 run_subagent 被只读白名单拦截→自述"我作为主 agent 直接执行审稿"→set_phase 返回"门禁未启用"（子代理本无 PhaseGate，同时解开 Ch28 pg 变 nil 之谜）→维护写入全被拦→输出主代理口吻总结而非审稿报告→ParseReport 失败（review_records #12=-1.0/unknown）。修复 A：RunSubAgent 在 instruction 尾部追加免疫声明（主历史中的 <system-reminder> 都是发给父代理的/无 run_subagent+set_phase+写权限/只执行指令任务），位于上下文最末近端位置最大化对抗力，~100 token 仅 miss 尾部；修复 C：agent.go set_phase 成功路径的阶段清单注入包进 if from != targetPhase——autoAdvancePhase 已注入过同一份静态清单，同阶段重复 set_phase 导致每转场注入两遍（write/review/maintain/done 均 ×2），纯浪费且破坏前缀缓存。B 方案（过滤 fork 中 <system-reminder>）评估后未实施：实测过滤点靠前会截断前缀缓存，子代理每次多 miss ~80-90K。业界佐证：claude-code#57751 同款问题（fork 式缓存继承导致 plan-mode bleed + self-poisoning）；Claude Code 默认方案为隔离式子代理。
...
> 2026-08-23：防设定漂移架构加固（P0-P2，源自 novel2-drift-audit）。①NS 方向锚区块（agentcfg/novel_state.go buildDirectionAnchor）：每轮注入本卷范围红线/总纲类型承诺截断/未兑现爽点 top3/活跃禁忌 top3/书本设定优先于类型模板仲裁规则，数据缺失静默跳过；②台账防腐（timeline_tools.go update）：status=resolved 时 resolved_chapter_id 超过当前最大章节号直接拒绝并提示改用 pending+target_chapter；③check_story_consistency 新增 4 个程序化检查类型：ledger_integrity（伏笔 resolved 指向未来章节/弧线节点 completed 无 actual 证据）、beat_window（未来3章到期大爽点写前对齐）、scope_guard（本章不在任何卷范围内告警+弧线节点滞后/提前消耗>5章检测——运行期版 init 校验#9）、type_drift（8 章窗口动作场景<2 章=漂移嫌疑，复用 pacing_gap 标签体系——init 校验#6 运行期版）；④审稿标准 v3→27项：新增 #26 卷纲范围越界、#27 类型方向契合（手段承诺/preferences 禁忌/lore 反应阶梯三对照），builtin 与 skills/ 双同步，内核 review 行计数更新，identity.go 审稿子代理 check_types 扩容为五项新检查；⑤main-cmd-ruling 技能（manual）：用户对话纠偏一键沉淀为偏好禁忌（提取规则化表述→确认→create_preference）；⑥门禁关闭审稿缺席提醒（agent.go assistantMessagesSinceLastReview，阈值6条）：PhaseGateEnabled=false 且最近一次 run_subagent 后累计 ≥6 条主 agent 回复时注入 system-reminder 提示审稿或重开门禁。缓存协议不受影响：NS/提醒均为动态区尾部字节。
...
> 2026-08-20 移动端 bug 修复（tokenInput id 冲突 + 门禁状态显示）：mobile/index.html tokenModal 里 <span id="tokenInput"> 与 tokenPrompt 模板里 <input id="tokenInput"> id 重复，getElementById 返回文档树顺序第一个 span（span.value=undefined，.trim() 抛 TypeError），导致弹窗输入令牌点连接无反应（扫码正常因走不同路径）；修复：span 改名 tokenMissCount + app.js updateTokenModal 同步引用 + showTokenPrompt 改用 overlay.querySelector 局部查询（方案 B 防御）+ 保存后补 api('/api/health') 验证 + connectWS 重连。门禁状态显示：updatePhaseStatus() 此前被两处事件处理调用但函数从未定义（空调用），现实现为调 updateQuickBar()；updateQuickBar 门禁按钮 label 在启用且有 currentPhase 时显示阶段中文名（t('phase_'+phase)，复用已有 i18n 键），否则显示开关状态——零额外 UI 空间。3 文件。

> 2026-08-20 移动端消息渲染重构（工具调用独立卡片）：SSE/WS 两路 tool_call 事件此前把 🔧 ... 追加到 content 字符串，与 AI 正文混在一个气泡（用户反馈'一坨'）；改为 appendToolBadge(el, ev) 在 msg-body 内 bubble 下方独立渲染 .tool-calls 容器，每工具一张卡片（图标+名称+状态：… 执行中/✓ 完成/✗ 失败），按 tool_id 去重更新；CSS .tool-badge-item 轻量卡片样式（surface2 背景 + border + 状态色）；done 事件不再覆盖工具卡片（卡片是独立 DOM，updateStreaming 只改 bubble 和 thinking）。3 文件，+52/-7。

> 2026-08-20 token 削减 P0-P3（真实会话审计驱动）：P0 修复技能注入去重 bug——injectPhaseSkills / messagesContainContent 去重只比对 system 消息 c==content，LLM 手动 auto_skill_injection 返回的全文在 tool 消息（JSON 包裹）未被识别，导致同一技能全文重复注入（真机 sess_2_18cd45de8fef902c 写第17章 chapter-hook-enhanced 注入 2 次：手动 9.2K + 系统 5K，浪费 ~8.5K token）；改为 system/tool 消息 + Contains 子串匹配（tool JSON 包裹也能命中）。P1 get_writing_context 精简：timeline 100→30、recent_chapters 5→3、outline beats 去 importance 字段（description 同步）——prepare 单次省 ~3-5K miss。P2 kernel 门禁引导强化：write 步骤 7 后加 write→review 边界提示（进入 review 立即审稿，禁止调用维护工具，迷你维护必须在 set_phase(review) 前完成），三份同步（skills/builtin/~/.goink）——真机 9 次门禁错误调用每次引发 thinking 修正轮，是最大隐性输出成本。P3 NS 指纹窗口 1500→1000（novel_state.go + cacheprobe/sim.go 同步，1000>800 下限仍覆盖 3 章防重复）+ 批量字数提醒精简（155→45 字符，两处）。mcp_tools appearance_tools_test 缺表 fail 为预存问题。7 文件，go build ./... + agent/cacheprobe 测试通过。

> 2026-08-20 get_writing_context timeline 窗口化（timeline 优化 A）：writing_context_tools.go 的 timeline 段从 ListByNovel 无窗口分页（前 30 条，含大量未来章才回收的 resolved 规划条目）改为 ListByChapterRange(0, 当前+10) + 手动过滤——pending 全部保留（target ≤ 当前+10），resolved 只保留近 5 章内回收的（resolved_chapter ≥ 当前-5），远处已回收条目交 get_timeline/check_story_consistency 核对不占 context。真机数据佐证：旧实现 resolved 10 条里 7 条是 34-45 章的未来规划（当前写 17 章，占 timeline 段 54%）。cacheprobe table 实测查询 miss 每次降 516-810 tokens（续写单章 37514→36998、批量每次 -810），同步工具 description。1 文件，+Storage import 移除，go build + agent 测试通过。

> 2026-08-20 kernel 精简（B 方案）：skills/main-core-writing-kernel.md（+ builtin + ~/.goink 三份同步）14,199→12,670 chars（省 11% ≈ 917 tokens/会话首轮，常规轮从缓存价再省）——① 阶段技能表整表删除改为一行 Stage Skills 说明（表内技能名与 catalog（L3 自动注入）及 prepare/outline/write 各阶段段的技能清单三重复，纯冗余；门禁注入读 phase_gate_config 不读此表，删表不影响注入）；② 批量说明 4 大段引文压缩为 3 段要点（保留全部创作规则：循环 read 大纲/迷你维护 6 工具/每 3 章一致性+文笔自检/批末全批审稿+覆盖核对）；③ 补回表删损失的调度信息：write 段新增 6.8 写后自审步骤（read revision-pass + anti-ai-grade，原技能表 write后 行独有）。硬约束段保留（压缩后技能全文丢失时的兜底安全网，红线）。cacheprobe 续写单章 miss -516，无调用方依赖此表（skill/agent 测试通过）。

> 2026-08-20 kernel 结构化重构（第三轮，13254→10314 chars 再省 22%）：skills/main-core-writing-kernel.md 从'一大坨'重写为 7 节结构化文档——① 1.工作模式（流程总览表 + 技能加载规则统一节解决 prepare/outline/write 三处重复 + 工具调用规范）；② 2.阶段流程每阶段固定模板（目标→必读技能表→步骤→转出set_phase），prepare 9 项必查/maintain 7 查询+15 更新改表格化；③ 3.批量模式细则单列（循环/迷你维护/每3章自检/批末审稿）；④ 4.卷结构 5.关键实体判定（表格化）6.硬约束 7.工具规则。全部创作规则保留（dead_characters 补回 write 6.5 步），零删减；三份同步（root/builtin/~/.goink）hash 一致，go build + skill/agent 测试通过。

> 2026-08-20 kernel 结构化审计 + 修复：上一轮 6efb981 结构化重构（12,670→10,361 chars，累计较原始 14,199 省 27%）后做全量规则核对——用 git diff 旧版 vs 新版提取 34 个工具名 + 全部技能引用 + 创作规则逐一比对，确认跨重构零丢失（唯一瑕疵：maintain 第 13 项 create_reader_perspective_entry 被简写为 create/update_ 且『杜绝冗余条目』语义被压缩，已补回完整工具名 + 完整语义）；结论：当前 kernel 已达精简+结构化平衡点（调度骨架+方法论导航+兜底安全网三位一体），各阶段『必读技能已就绪』提醒/硬约束段/情景技能列表均是对抗压缩后技能丢失与创作方法导航的必要内容，再精简 <5% 且都触红线，不建议继续。2 文件（kernel 三份同步 + audit-log），go build + skill 测试通过。

> 2026-08-20 门禁注入 skill 精炼性审计（17 个全量）：已读全部 16 个 auto-inject 技能（init-phase 7670/chapter-title 3523/book-outline 3318/anti-ai 3278/data-hygiene 2251/foreshadow 2175/character-design 1832/show-dont-tell 1666/genre 1661/hook 1395/pov 1343/info-density 1130/world-building 1099/word-count 934/anti-repetition 699/common-sense 682）+ init-phase。结论：全部为创作方法论本体（触发时机+规则框架+正反例表格+自查清单），零纯散文填充，不可精炼。子代理建议的 4 处'可压缩'经逐句核实全部否决：①character-design L12 触发提示（删损失加载锚点）②info-density L42/L62 引言与节奏总结（各有引导功能）③anti-repetition L42-46 格式禁止性补充（代码块之外的约束，非复述）④'铁轨/火车'跨技能重复（common-sense 注 prepare、world-building 注 init，不同阶段注入各自需自包含，删一处另一端失去约束）。与 kernel 不同——kernel 是调度骨架可结构化去重（上一轮 14,199→10,361 省 27%），技能是方法论本体无冗余。继续压收益 <2% 且全触创作红线，不做。
...
> 2026-08-21 章节/大纲手动删除（桌面端 UI）：前端缺一键删除章节/大纲功能，此前章节只能创建/改名，大纲文件无法删除。新增——git/rw.go DeleteFile（os.Remove，文件不存在返回 nil 幂等）；chapter/store.go Delete（novel_id + chapter_number 删 DB 记录）；app/chapter.go DeleteChapter（删正文文件 + DB 记录）与 DeleteOutlineFile（删 outlines/NNN.md 两个 Wails 绑定）；ChapterList.tsx 每章/每大纲条目右侧加 🗑️ 按钮（hover 显示，与编辑笔同行），window.confirm 确认后删除并刷新列表；useApp.ts 重新导出两个新方法。设计决策：不自动重编号（删 5 章 6 章号不变）、不清理关联数据（scene/timeline 防误删）、不自动 git commit（与 CreateChapter 一致）；仅桌面端（HTTP 端点不需要）。5 文件，go build + 前端编译通过，新 exe 已部署。

> 2026-08-21 全量审计修复（安全/正确性/漂移收敛，P0-P2 共 30+ 项）：移动端 XSS 链路修复（marked.use 覆写 html/link 渲染器转义原始 HTML + 限 http(s) 链接 + index.html CSP meta + esc() 补引号转义）；移动端 /api/chat 字段名 provider_name/model_id 改为服务端实际解析的 model/provider（选模型从未生效的根因）；移动端离线短路改连续 2 次失败才启用 + 30s TTL 自动恢复探测 + 超时 500ms 调 2.5s；sw.js v2（预缓存补 idb-keyval.min.js、静态资源回填、API 只缓存 GET 带 10min TTL、跳过 POST）；WS 重试预算耗尽（RetriesExhaustedError）后 10s 整体重建连接；copyText 形参 t 遮蔽 i18n 函数修复；sendMessage 补 res.ok 检查；syncToOffline 书架指纹去重防 N+1 全量重拉；openNovel 清 chaptersCache；novelDetailHeader 删除按钮改 addEventListener 防属性注入；manifest 补 SVG 图标。前端 ChatPanel：发送代际号守卫（并发发送旧 finally 不再误删新监听器）、切换会话/新对话/切小说三入口拆除动态监听并清事件队列（turn_id 撞号污染根治）、handleRetry 移出 setTurns updater（StrictMode 双发双倍计费）、补 chat:title_updated 监听刷新标题、删除活跃会话回传 ID 后 handleNewChat 善后、工具卡片合并计数 lastHeadId 回溯修 ≥3 连段少报、MessageBubble 移除死时间戳 prop、RetryNotification onDone 稳定引用防倒计时重置。CachesimView 与 cache_sim.go：StartCacheSimScenarios 加 reqID 参数随 batch-done 事件带回，前端按最新请求过滤（预设/自定义跑批结果不再互相覆盖），wails generate module 已重跑。ContextRing 价格输入与压缩阈值 600ms 合并防抖落库。PhaseGateConfigTab/GeneralConfigTab 拦截空值保存假成功。i18n 补根级 selectAll 与 location.fitView。WorkspaceView renderSpecialPanel 单次调用。门禁：batch write 章边界新增 check_story_consistency 强制（写时把关，toolAvailable 守卫防自定义配置焊死）+ 3 个测试更新 + 新增 2 个边界测试。mcp_tools：findVanishedCharacters 改单条 JOIN 三列查询、findDeadAppeared 改单次遍历每角色报最早一次、删除 findInitConsistency 死代码循环（prefs 加载移至 taboo_violation 前）。skill 技法：revision-pass 八条铁律改九条、maliang-method 金手指亮相时间线对齐 golden-three-chapters、anti-ai-grade T1 加万能连接适用范围豁免、pov-purity POV 标注改场景分隔措辞、genre-templates 注明爽点密度层级口径。文档：门禁配置示例.md 重写为 default_phase_gate_config.go 的字节级镜像并声明权威源、AGENTS.md skills/ 目录描述更新为 6 文件现状、identity.go maintain 口径统一为 14 项维护+阶段切换、API.md 补 POST/DELETE novels 与 sync/state 端点及 WS room 参数并修正重复章节编号。验证：go build 通过，agent/skill/app 测试全绿（mcp_tools 3 个失败为基线预存已 stash 对照确认），tsc+vite 构建通过。

> 2026-08-21 skills/ 演进版回灌 builtin（单一权威源收敛）：三对文件逐行 diff 核对后回灌——main-core-init-phase（一行对齐：init 阶段 edit 不再写 book-outline.md，总纲走 DB）与 sub-tech-review-standards（v2 纯增强：第 23 项收紧为连续 2 章警告/3 章致命、新增第 24 项场景类型轮换、原爽点兑现比对升级为第 25 项改读 get_outline 的 outline_beats）直接覆盖；main-core-writing-kernel 为结构重写版，diff 发现 skills 版缺 builtin 三条规则，合并回灌而非直拷——补回 init 小节（9 步流程 + 用户未明确确认禁止 set_phase 硬性门槛）、write 第 7 步单章写后自审（revision-pass + anti-ai-grade）、write→review 边界说明（进入 review 禁维护工具）。顺手修正 skills 版自查清单第 6 条陈旧引用（爽点承诺未兑现已从第 24 项移到第 25 项）。回灌后 6 文件与 builtin 逐字节一致（md5 校验），AGENTS.md 同步状态描述更新。红线核对：零删减，仅增加或改写表达。验证：go build + skill 包测试通过。

> 2026-08-23 桌面端 UI 改造（低端机性能红线内，禁 backdrop-filter、动画只 transform/opacity）：① Markdown 表格自适应渲染——Markdown.tsx 新增 table override（AdaptiveTable + tableStructure：遍历 children 用 getNodeText 提取 headers/rows），列数 ≤3 正常表格包 .markdown-table-scroll（overflow-x:auto + nowrap + min-width:max-content），列数 ≥4 降级为逐行卡片（.markdown-table-cards/.markdown-table-card，字段名做 label），纯 CPU 零开销；Markdown.css 补对应样式。② 底部三控件收进输入框卡片——ChatInput.tsx 新增 controls?: ReactNode prop 渲染在输入卡内部底行（mt-1 -mx-2 border-t 分隔）；ChatControls.tsx 新增 embedded?: boolean（嵌入态去 px-4 py-2 外边距）；ChatPanel.tsx 将 ChatControls 作为 controls prop 传入 ChatInput，删除独立控件行与 border-t 分隔线。③ ChapterList 标题前缀归一化——新增 stripChapterPrefix（	itle.replace(/^第\s*\d+\s*章\s*[·:：-]?\s*/, ''))，应用于章节标题（原 L340）与大纲标题（原 L257）渲染，修复「第2章 第2章」重复显示（根因：AI 把含前缀文本存入 title，前端编号 span 又渲染一次）。ActivityBar 激活指示条/分组线/tooltip 检查后确认已实现，未改动。验证：tsc+vite 构建通过，goink-20260823-13-08.exe 部署 D:\Goink\。Commit 77fab91。
