...
> 2026-08-09：全量审计工具范围参数（用户要求）——17 个查询工具逐一核对：15 个支持范围/指定/分页取数（get_writing_context current_chapter 树状、get_timeline/arcs 窗口、get_scenes chapter_id、get_characters search/brief/分页、get_lore/items/locations mode+ID 指定、get_entity_appearances 实体维度、search_* limit），仅 2 个只有全量路径，已补范围参数：**get_item_occurrences 加 chapter_from/chapter_to**（章节范围，join chapters 表按章号过滤，store ListByItemRange + 测试）；**get_reader_perspective 加 search/planted_from/planted_to**（内容关键词定向 + 种植章节范围，store ListActiveFiltered + 测试）；两个工具 Description 明确"禁止无范围/无过滤拉全量（上限 50/100 条）"；get_item_occurrences 范围返回带 chapter_number 语义
