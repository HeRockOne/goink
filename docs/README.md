...
> 2026-08-09：批量质量补齐验证——批量流程 vs 单章的质量差距点确认（单章每章有 write 后自审 selfReviewPlays，批量 write 循环内没有；批量 review 只审第 1 章）；batchGatePlays 拆出 batchGatePlaysWith(chapters, selfReview)；模拟实测批量 5 章补齐每章自审：成本 ¥0.0915→¥0.1050/章（+14.7%），对比单章 5 轮 ¥0.2751/章仍省 61.8%——批量省钱与质量补齐可兼得，业务改进方向为批量 write 循环加每章自审（待真机验证后实施，业务代码未动）
