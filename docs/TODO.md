...
2. **压缩触发点模拟**（cacheprobe 目前无压缩模拟，README 注明"模拟未含压缩重置"）：
...
## 结论速查（质量 × 成本，批量 5 章，2026-08-09 实测）

质量分 = 写后自检(3) + 审稿覆盖×3 + 状态实时(2) + 写前对齐(1) + 章纲/防串章(1)，满分 10。

| 方案 | 成本¥/章 | 省vs单章 | 审稿覆盖 | 自检节奏 | 质量分 | 质量/成本 |
|---|---|---|---|---|---|---|
| 单章 5 轮（基准） | 0.2750 | 0% | 100% | 每章 | 9.0 | 32.7 |
| 批量现状（攒批统一审） | 0.0886 | 67.8% | 20% | 无 | 4.1 | 46.3 |
| 批量 + 每章轻量自检 | 0.1050 | 61.8% | 0% | 每章 | 6.5 | 61.9 |
| 批量 + 三章一轮·轻量自检 | 0.0918 | 66.6% | 0% | 每3章 | 6.0 | 65.4 |
| 批量 + 三章一轮·批内检查 | 0.0971 | 64.7% | 60% | 每3章 | 7.8 | 80.3 |
| 批量 + 三章一轮·完整门禁流程 | 0.1110 | 59.6% | 100% | 每3章 | 9.0 | 81.1 |

解读：
- **三章一轮·批内检查是性价比最优**（质量 7.8/成本 0.0971）：比现状仅贵 9.6%，审稿覆盖 60%→100% 提升空间靠批量更大（6 章时第 3/6 章触发，覆盖 100%）
- 要满分质量选**完整门禁流程**（9.0 分，+25% vs 现状，质量/成本 81.1 最高）
