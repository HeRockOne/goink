...
> 最后更新：2026-08-05（默认主题改为「太虚」青蓝水墨系，新增扩展 token）
> 2026-08-07：特效系统已移除（commit d032284），文档同步删除「八、特效系统」；粒子变量为遗留死代码

---

## 〇、默认主题：太虚（青蓝水墨）

2026-08-05 起内置默认主题从牛皮纸改为太虚风格（仿"太虚剑宗"）：

- **深色**：深蓝黑背景 `#0a0e17` + 剑宗青 `#a1c4d6` 主色，半透明毛玻璃面板（`backdrop-blur`），细透明边框，背景层含渐变 + 斜线纹理 + 山峦剪影
- **浅色**：水墨玉白 `#eef3f8` + 太虚蓝 `#6b8fad`
- 标题使用书法字体（楷体 KaiTi，Windows 自带零依赖）
- 光感：激活项/标题发光（`--glow` token）、叙事按钮 active 发光、分隔线渐变
...
| 变量 | 用途 | 浅色 | 深色 |
|------|------|------|------|
| `--reader-bg` | 阅读器整体背景 | `#eef3f8` | `#0a0e17` |
| `--reader-paper` | 阅读器纸面 | `#ffffff` | `#121a28` |

### 2.10 太虚扩展 token（2026-08-05 新增）

| 变量 | 用途 | 浅色 | 深色 |
|------|------|------|------|
| `--glow` | 通用发光色（光晕/文字辉光） | `rgba(107,143,173,0.18)` | `rgba(161,196,214,0.14)` |
| `--glow-strong` | 强发光（激活项文字辉光） | `rgba(107,143,173,0.35)` | `rgba(161,196,214,0.3)` |
| `--font-display` | 标题书法字体（楷体） | KaiTi/STKaiti/楷体 | 同左 |
| `--particle-color` | 粒子主色（遗留，无渲染器） | `rgba(74,123,156,0.4)` | `rgba(161,196,214,0.5)` |
| `--particle-glow` | 粒子光晕（遗留，无渲染器） | `rgba(74,123,156,0.12)` | `rgba(161,196,214,0.16)` |
| `--particle-line` | 粒子连线/剑形线（遗留，无渲染器） | `rgba(74,123,156,0.15)` | `rgba(161,196,214,0.18)` |
| `--bg-layer-grad` | 全局背景层渐变（三层径向+线性） | 水墨玉白 | 深蓝黑 |
| `--mountain1..5` | 山峦剪影五层（背景层底部） | 浅灰蓝 | 深蓝黑 |

> 布局层级：`bg-layer(z-0)` → 工作区内容/分隔线(z-5) → 叙事面板(z-8) → 叙事面板工具栏(z-15) → 模态框(z-999)。面板用半透明 `--sidebar`/`--card` + `backdrop-blur`，编辑器区保持实底高对比。
> 特效层（旧 z-10）已随特效系统移除（d032284），`--particle-*` 变量仅存留于 `index.css`，无任何渲染器使用。
...
- `effects` 字段为旧格式遗留（特效模块已移除，d032284），读取时忽略（`useTheme.ts` 明确注释）
...
- **质感层跟随主色**（2026-08-15 起）：`--bg-layer-grad`（全窗口背景渐变）从 `--primary` 径向渐变 + `--background` 基底派生；`--glow/--glow-strong`（标题辉光/激活发光）从 `--primary` 调透明度派生——否则自定义主题会残留内置太虚青蓝背景层与发光，与新配色打架
...
内置默认主题即为「太虚·夜」（深色）。也可用生成器复现：深色模式 + 背景 `#0a0e17` + 主色 `#a1c4d6`。完整 JSON 见「填入示例主题」按钮（`ThemeConfigTab.tsx` 的 `INITIAL_THEME_JSON`）。
