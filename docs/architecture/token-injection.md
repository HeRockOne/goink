...
> 注意：tokencount 扫描 `skills/` 目录 + `internal/skill/builtin/`。内置 42 个 skill（含 8 新增）通过 `//go:embed` 加载，其中 38 auto 进入 catalog，4 manual 不出现。以下实测为 tokencount 直接输出，实际总注入另加内置 catalog 约 1,152 tokens。
...
├─ 工具定义（59 个工具的完整 JSON Schema）  12,924 (76.3%)
...
| 组成部分 | Tokens | 占比 | 说明 |
|---------|--------|------|------|
| 工具定义 | 12,924 | 76.3% | 59 个工具 name + description + parameters schema（含 `$defs` 内联） |
...
L1  Identity        → 1,340 tokens   人设/流程/规范（agentcfg/identity.go）
	...
	L3  Skill catalog   → 525+1,152 tokens   auto 模式 skill 的 name+description 目录（skills/ 8 auto + builtin 38 auto）
...
3. **skill 正文不占常驻 token** → 38 个 auto skill 只注入 ~1,150 tokens 的目录，正文按需加载，这是省 token 的核心策略
...
| Catalog 瘦身 | 少量 | 38 个 auto skill 目录压缩 | 未实施 |
