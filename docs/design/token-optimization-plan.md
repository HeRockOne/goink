...
> 当前系统提示词注入构成已用 `tokencount` 精确统计，见 **`architecture/token-injection.md`**（~17,500 tokens / 57 工具 / 12,924 工具定义）。
...
**预计节省**：52 工具 × 100 token = **-5,200 token/轮**
...
|------|-----------|---------|----------|----------|
| 工具数 | 52 | <20（建议） | 2-32（按阶段） | 2-32（按阶段） |
